                                            ...
                                            vector<vector<ITEM_ID> > &grid)
{
   ...
   if(strategy == LEE_STRATEGY::SHORTEST_PATH)
   {
      ...
      for(auto indexX = x_l; indexX <= x_h; ++indexX)
      {
         ...
         {
            if(grid[toGridIndex(indexY)][toGridIndex(indexX)] > 0)
            {
//               cout<<"grid[indexY][indexX]="<<grid[indexY][indexX]<<endl;
               ...
               if(temp < min_l)
               {
                  ...
                  min_x = indexX;
                  min_y = indexY;
                  step_num_partial = grid[toGridIndex(indexY)][toGridIndex(indexX)];
               }
            }
         }
      }
...
{
   auto x = toGridIndex(ix);
   auto y = toGridIndex(iy);
   float density = (*pDensity)[y][x] * 4;
   if(iy + 1 < static_cast<int>(pDensity->size()))
      density += (*pDensity)[y + 1][x] * 2;
   if(iy - 1 >= 0)
      density += (*pDensity)[y - 1][x] * 2;
   if(ix + 1 < static_cast<int>(pDensity->at(0).size()))
      density += (*pDensity)[y][x + 1] * 2;
   if(ix - 1 >= 0)
      density += (*pDensity)[y][x - 1] * 2;
